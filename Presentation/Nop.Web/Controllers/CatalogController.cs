...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BaseNopController
    {
        ...
        [NonAction]
        protected IList<CategoryNavigationModel.CategoryModel> PrepareAllCategoriesNavigationModel(int rootCategoryId)
        {
            var result = new List<CategoryNavigationModel.CategoryModel>();
            foreach (var category in _categoryService.GetAllCategoriesByParentCategoryId(rootCategoryId))
            {
                ...
                categoryModel.SubCategories.AddRange(PrepareAllCategoriesNavigationModel(category.Id));
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            model.PictureModel = new PictureModel()
            {
                ...
                ImageUrl = _pictureService.GetPictureUrl(category.PictureId, 795),
        ...
        [ChildActionOnly]
        public ActionResult AllCategoriesNavigation(int currentCategoryId, int currentProductId)
        {
            //get active category
            var activeCategory = _categoryService.GetCategoryById(currentCategoryId);
            if (activeCategory == null && currentProductId > 0)
            ...
            var customerRolesIds = _workContext.CurrentCustomer.CustomerRoles
                .Where(cr => cr.Active).Select(cr => cr.Id).ToList();
            var cacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_ALL_NAVIGATION_MODEL_KEY, _workContext.WorkingLanguage.Id,
                string.Join(",", customerRolesIds), _storeContext.CurrentStore.Id);
            var cachedModel = _cacheManager.Get(cacheKey, () =>
            {
                return new CategoryNavigationModel()
                {
                    Categories = PrepareAllCategoriesNavigationModel(0).ToList()
                };
            }
            );
